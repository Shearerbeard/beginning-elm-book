...
inverterTests : Test.Test
inverterTests =
    describe "Inverter"
        [ test "output is 0 when input is 1" <|
            \() ->
                inverter 0
                    |> Expect.equal 1
        , test "output is when when the input is 0" <|
            \() ->
                inverter 1
                    |> Expect.equal 0
        , test "output is -1 when input is neither 1 nor 0" <|
            \() ->
                inverter 3
                    |> Expect.equal -1
        ]
    ...
    describe "AND gate"
        ...
        , test "output is 0 when the first input is 0" <|
            \() ->
                andGate 0 1
                    |> Expect.equal 0
        , test "output is 0 when second input is 0" <|
            \() ->
                andGate 1 0
                    |> Expect.equal 0
...
rippleCarryAdderTests : Test.Test
rippleCarryAdderTests =
            ...
            [ test "and carry-in is 0, the output is 0000" <|
                \() ->
                    rippleCarryAdder 0 0 0
                        |> Expect.equal 0
            , test "and carry-in is 1, the output is 00001" <|
                \() ->
